...
sessions via a Unix socket daemon. Claude Code plugin hooks notify the daemon of
session state changes; the TUI subscribes for live updates.

### Workspace

| Crate                     | Purpose                           | Binary                     |
| ------------------------- | --------------------------------- | -------------------------- |
| `agent-console-dashboard` | Main app: daemon, TUI, CLI        | `acd`                      |
...
```text
Claude Code plugin hook fires
  → acd claude-hook <status> (reads JSON stdin, extracts session_id)
...
- D4: Claude Code plugin system for hook management (replaces shell scripts + jq)
...
- `acd claude-hook` subcommand reads JSON stdin and forwards to daemon
- Claude Code plugin manifest generated by build.rs
...
- **Integration tests**: `acd claude-hook` subcommand needs automated
  integration tests (see acd-80s).

## Next Steps

- End-to-end testing with real Claude Code plugin system
- Integration tests for `acd claude-hook` (acd-80s)
