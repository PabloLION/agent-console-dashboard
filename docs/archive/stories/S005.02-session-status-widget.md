...
yellow for attention, and gray for closed.

## Implementation Details

### Technical Approach

1. Create `crates/agent-console-dashboard/src/widgets/session_status.rs`
2. Implement Widget trait for SessionStatusWidget
3. Support horizontal format: `proj-a: ● | proj-b: 2m34s | proj-c: 1m12s`
...
- `crates/agent-console-dashboard/src/widgets/session_status.rs` -
  SessionStatusWidget implementation
...
- Session detail view (S004.04 in E004)
...
```text
proj-a: ● | proj-b: 2m34s | proj-c: 1m12s
```
...
### Elapsed Time Seconds Omission

For durations >= 1 hour, seconds are omitted for readability (e.g., `2h5m` not
`2h5m0s`). This is intentional — at hour-scale durations, second-level precision
adds visual noise without meaningful information.
