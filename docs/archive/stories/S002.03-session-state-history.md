...
From [E002](../epic/E002-session-management.md):
...
4. Implement configurable history depth limit (default: 100 entries)
...
- `crates/agent-console-dashboard/src/daemon/session.rs` - Add history recording
  methods
- `crates/agent-console-dashboard/src/config.rs` - Add history depth
  configuration option

### Dependencies

- [S002.01 - Session Data Model](./S002.01-session-data-model.md) -
  StateTransition struct must be defined
...
impl Session {
    ...
    fn record_transition(&mut self, from: Status, to: Status, duration: Duration) {
        ...
        // Enforce depth limit (configurable, default 100)
...
Same-status transitions (e.g., Working -> Working) update the `since` timestamp
without creating a history entry, per S002.02. This avoids polluting history
with redundant entries when hooks re-send the current status.

### History Depth Config Location

`history_depth_limit` is a global config value under `[sessions]` in TOML. The
Session struct reads this value from the global config at creation time.

### Concurrency Note

With the **single-threaded actor model**, history updates are sequential. No
race conditions. See [concurrency model](../architecture/concurrency.md).
