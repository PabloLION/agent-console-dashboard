...
- [ ] Given session working directory, then `claude --resume` runs with `cd <dir> &&` pattern
...
> **Note:** Zellij environment detection is handled by S010.03's
> `TerminalEnvironment::detect()`. S010.02 consumes the detection result.

```rust
// Detection is provided by S010.03's TerminalEnvironment::detect()
// S010.02 consumes it rather than implementing its own is_inside_zellij()
```
...
fn handle_resurrect_key(&mut self, session_id: &str, working_dir: &Path) -> Result<()> {
    if zellij::is_inside_zellij() {
        ...
        let cmd_str = format!("cd {} && claude --resume {}", working_dir.display(), session_id);

        match zellij::create_pane_with_command("sh", &["-c", &cmd_str]) {
            Ok(_) => {
                // Notify daemon
            ...
            Err(e) => {
                ...
                let cmd = format!("cd {} && claude --resume {}", working_dir.display(), session_id);
