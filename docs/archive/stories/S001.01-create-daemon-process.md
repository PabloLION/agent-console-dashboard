...
> **Note:** The daemon is NOT considered idle while any TUI subscriber is
> connected. The auto-stop timer only starts when both conditions are met: zero
> active sessions AND zero TUI subscribers.

> **Note:** SIGHUP handler is deferred to E007 (hot-reload configuration).
> S001.01 only handles SIGTERM and SIGINT for graceful shutdown.
