...
4. Show API usage summary if available (from WidgetContext)
...
- `crates/agent-console-dashboard/src/tui/views/mod.rs` - Add detail view export
- `crates/agent-console-dashboard/src/tui/views/detail.rs` - Session detail view
  component
- `crates/agent-console-dashboard/src/tui/app.rs` - Add view state
  (Dashboard/Detail), transition logic
- `crates/agent-console-dashboard/src/tui/ui.rs` - Render detail view when
  active
- `crates/agent-console-dashboard/src/tui/event.rs` - Handle detail view
  keyboard events
...
- [S004.01 - Ratatui Application Scaffold](./S004.01-ratatui-application-scaffold.md) -
  TUI foundation
- [S004.02 - Main Dashboard Layout](./S004.02-main-dashboard-layout.md) - Main
  view to overlay
- [S004.03 - Keyboard Navigation](./S004.03-keyboard-navigation.md) - Enter key
  triggers detail
...
- [ ] Given detail view is open, when rendered, then API usage summary is
      displayed (if available from WidgetContext)
- [ ] Given detail view is open, when rendered, then state history timeline
      shows recent transitions with format: `HH:MM:SS PreviousState → NewState`
- [ ] Given history has more than 5 entries, when rendered, then only the most
      recent 5 are shown with a scroll indicator
...
> **Note:** Help view is deferred to v1+.
...
Usage data comes from `WidgetContext.usage` (populated from daemon broadcasts).
Format matches the api-usage widget from E005:

- Full format if width allows: `Quota: 5h 8% | 7d 77%`
- Compact format for narrow modals: `[5h:8% 7d:77%]`
- Placeholder if unavailable: `Quota: --`

See [widget data flow](../architecture/widget-data-flow.md) for data source
architecture.
