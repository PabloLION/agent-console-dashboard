...
# Initial Response
{"type":"STATE","sessions":[{"id":"abc123","status":"working","status_elapsed":0}]}

# Subsequent Updates (pushed by daemon)
{"type":"UPDATE","session":"abc123","status":"working","status_elapsed":0}
{"type":"UPDATE","session":"abc123","status":"attention","status_elapsed":45}
{"type":"UPDATE","session":"def456","status":"attention","status_elapsed":5}

# On session removal
{"type":"UPDATE","session":"abc123","status":"closed","status_elapsed":120}
```

### Files to Modify

- `crates/agent-console-dashboard/src/daemon/server.rs` - Add SUBSCRIBE handler
  and subscriber management
- `crates/agent-console-dashboard/src/daemon/store.rs` - Add subscriber
  notification on state changes
- `crates/agent-console-dashboard/src/daemon/mod.rs` - Add subscriber list type
...
- [ ] Given multiple subscribers, when a session changes, then all subscribers
      receive UPDATE
- [ ] Given a subscriber disconnects, when the next UPDATE is broadcast, then
      the failed send removes the subscriber from the list
- [ ] Given the connection is open, then it remains open indefinitely (no
      timeout)
- [ ] Given an UPDATE, then format is
      `UPDATE <session> <status> <status_elapsed>\n`
...
```rust
// Subscriber list in DaemonState
struct DaemonState {
    sessions: HashMap<String, Session>,
    subscribers: Vec<mpsc::Sender<String>>,
}

// When session changes, event loop calls this
fn broadcast_update(state: &mut DaemonState, update: &str) {
    // Send to all, remove failed sends (bounded channel per D1)
    state.subscribers.retain(|tx| tx.try_send(update.to_string()).is_ok());
}
```

Per [concurrency model](../architecture/concurrency.md), use bounded channels
(size 100) to prevent slow TUIs from blocking broadcasts to other TUIs.

### UPDATE Message Format

```text
UPDATE <session_id> <status> <status_elapsed>
```

- `session_id`: The session that changed
- `status`: New status (working, attention, closed)
- `status_elapsed`: Seconds in current status at time of transition
