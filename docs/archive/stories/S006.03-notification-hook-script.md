...
All notification types (including questions) map to the "Attention" status. The
Status enum is: Working, Attention, Closed. There is no separate "Question"
status -- AskUserQuestion events map to "Attention".
...
v2.0.76. AskUserQuestion events map to "Attention" status (not a separate
"Question" status). The Status enum is: Working, Attention, Closed.
...
> **Note:** Read BOTH `session_id` from JSON stdin (`jq -r '.session_id'`) and
> `$CC_SESSION_ID` env var. Compare them -- if different, log both for
> debugging. Primary mechanism is JSON stdin.
