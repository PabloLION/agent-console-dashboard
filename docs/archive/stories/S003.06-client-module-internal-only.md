...
The `client` module (`crates/agent-console-dashboard/src/client/`) provides
internal connection functionality for:
...
- [ ] `crates/agent-console-dashboard/src/client/mod.rs` documents that the
      module is internal-only
- [ ] `crates/agent-console-dashboard/src/lib.rs` does NOT export the client
      module
...
- `crates/agent-console-dashboard/src/lib.rs` - Ensure no `pub mod client`
  export
...
- `pub(crate)` visibility is correct — the client module should be accessible
  within the crate but not exported as a public API. The concern is only about
  preventing accidental `pub` exports
...
- [ ] Unit test: verify `client` module is not publicly exported from `lib.rs`
- [ ] Manual test: confirm `cargo doc` does not list client module in public API

## Out of Scope

- Hiding other internal modules (address individually as needed)
- Publishing stable public API guarantees
