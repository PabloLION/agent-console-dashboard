...
> **Note:** The `mpsc::channel(100)` used for subscriber broadcasts is a
> per-subscriber message buffer, independent of the max concurrent clients
> count. When a send to a subscriber fails (buffer full or disconnected), that
> subscriber is removed. See [S003.04](./S003.04-subscribe-command.md) for
> broadcast details.
