...
Once closed session metadata is retained (S8.1), users need a way to trigger the
...
- [S8.1 - Closed Session Metadata](./S8.1-closed-session-metadata.md) - Provides
  closed session data
- [S3.1 - IPC Message Protocol](./S3.1-ipc-message-protocol.md) - Defines
  protocol structure
- [S3.5 - CLI Client Commands](./S3.5-cli-client-commands.md) - CLI
  infrastructure
- [S4.3 - Keyboard Navigation](./S4.3-keyboard-navigation.md) - TUI keyboard
  handling
...
- Actual invocation of `claude --resume` (handled in S8.3)
...
impl ResurrectCommand {
    pub fn execute(self, store: &mut SessionStore) -> Response {
        ...
        // 4. Prepare resurrection info (actual invocation in S8.3)
