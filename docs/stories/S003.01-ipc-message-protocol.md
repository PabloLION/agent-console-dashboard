...
{"type":"USAGE_UPDATE","five_hour_pct":8.0,"seven_day_pct":77.0,"reset_time":"2026-01-22T09:00:00Z"}
...
- [ ] Given a valid RM command JSON, when parsed, then returns `Command::Remove`
...
- [ ] Given a `Response::Error`, when serialized, then produces JSON with error
      message and `\n` terminator
- [ ] Given a `Response::UsageUpdate`, when serialized, then produces JSON with
      usage percentages and reset time
- [ ] Given a `Response::UsageUnavailable`, when serialized, then produces JSON
      indicating usage data is unavailable
...
pub enum Response {
    ...
    #[serde(rename = "USAGE_UNAVAILABLE")]
    UsageUnavailable,
}
```
...
These complexity considerations are addressed during implementation of this
story. No separate tracking needed.
