...
- [ ] Given closed sessions exceed max_closed_sessions config (default 20), when
      cleanup runs, then oldest closed sessions are removed first
...
When a SET command arrives for an unknown session ID, the daemon auto-creates a
session with the following metadata:

| Field        | Source                      | Initial Value               |
| ------------ | --------------------------- | --------------------------- |
| session_id   | `session_id` from hook JSON | Required                    |
| status       | From SET command            | `working` (initial default) |
| working_dir  | `cwd` from hook JSON stdin  | Falls back to "unknown"     |
| created_at   | Current timestamp           | `Instant::now()`            |
| display_name | Basename of `working_dir`   | Falls back to "unknown"     |
