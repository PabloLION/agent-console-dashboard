...
- [ ] Given Claude Code sends a notification, when the hook runs, then
      `agent-console set <project> attention` is called
- [ ] Given Claude Code is running in `/home/user/code/frontend`, when
      notification fires, then project name is `frontend`
...
Claude Code may also have an AskQuestion hook that fires specifically when
Claude uses the AskUserQuestion tool. This hook is currently not well-documented
or may not be available. If it becomes available, a separate story should be
created to handle it with a "Question" status.
