...
- [ ] Manual verification: terminal restoration on panic is verified manually,
      not in CI. Unit tests verify the panic hook is installed.
...
```text
crates/agent-console-dashboard/src/
...
S004.01 scaffolds the event loop with terminal input handling. S004.02 adds the
daemon IPC arm to the `select!` loop.
