...
failure with a fixed 5-second restart delay.
...
- [ ] `RestartSec=5` for fixed 5-second restart delay
- [ ] `WantedBy=default.target` for auto-start on login
- [ ] `systemctl --user enable acd` succeeds when unit file is copied to
      `~/.config/systemd/user/`
- [ ] Given the service is enabled AND started (`systemctl enable --now`), when
      the system is running, then the daemon is active
...
- [ ] Manual test: verify daemon starts after `enable --now`
...
When run via systemd, the daemon runs in foreground mode (`Type=simple`).
systemd manages the process lifecycle itself.
...
5-second fixed delay prevents rapid restart loops if the daemon has a persistent
configuration error or startup failure. Actual exponential backoff is
