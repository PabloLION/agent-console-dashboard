...
6. Implement layout switching with count keys 1-4
7. Add q for quit
8. Support arrow keys as alternative to j/k

### Files to Modify

- `crates/agent-console-dashboard/src/tui/event.rs` - Add keyboard event
  handlers
- `crates/agent-console-dashboard/src/tui/app.rs` - Add selection state, action
  handlers
- `crates/agent-console-dashboard/src/tui/views/dashboard.rs` - Highlight
  selected session
- `crates/agent-console-dashboard/src/tui/ui.rs` - Update render to show
  selection
...
- [S004.02 - Main Dashboard Layout](./S004.02-main-dashboard-layout.md) -
  Session list rendering
- [S008.02 - RESURRECT Command](./S008.02-resurrect-command.md) - For resurrect
  action (optional, can stub)
...
| `q`       | Quit                      | Any                     |
| `Esc`     | Close overlay/go back     | Detail view             |
...
pub fn handle_key_event(app: &mut App, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char(c @ '1'..='4') => {
            let layout = c.to_digit(10).unwrap() as usize;
