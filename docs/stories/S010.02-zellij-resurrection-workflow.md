...
- `crates/agent-console-dashboard/src/cli/commands/resurrect.rs` - Add Zellij
  integration flag
...
- [S010.03 - Claude Resume in Terminal](./S010.03-claude-resume-in-terminal.md) -
  Claude Code resume command execution
...
- [ ] Given session is resurrected, then dashboard status updates to "Working"
      within 2 seconds
- [ ] Given user runs resurrection with `--pane-id 2`, then the resume command
      is sent to pane 2 and that pane is focused
