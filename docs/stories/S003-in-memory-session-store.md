...
As a daemon,
I want to store session state in memory using a HashMap,
So that I can track all active agent sessions efficiently.

## Context

The session store is the in-memory data structure that holds all session state. It uses a HashMap for O(1) lookups by session ID. The store must be thread-safe since multiple async tasks may access it concurrently (socket handlers updating state, subscribers reading state).
...
- State lost on crash is acceptable (sessions refresh quickly through normal use)
...
- S001: Daemon process provides the runtime context

## Acceptance Criteria

- [ ] Given a new session ID, when `set` is called, then a new session is created in the store
- [ ] Given an existing session, when `get` is called with its ID, then the session data is returned
- [ ] Given an existing session, when `set` is called with updated status, then the session is updated
- [ ] Given an existing session, when `remove` is called, then the session is removed from the store
- [ ] Given multiple sessions, when `list_all` is called, then all sessions are returned
- [ ] Given concurrent access from multiple tasks, when operations occur simultaneously, then no data corruption occurs
- [ ] Given daemon running with sessions, when memory is measured, then RAM usage is under 5MB
...
- State history tracking (S007 - Session State History)
