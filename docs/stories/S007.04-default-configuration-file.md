...
- `crates/agent-console-dashboard/src/config/default.rs` - Create default configuration template
- `crates/agent-console-dashboard/src/config/loader.rs` - Add logic to create default file on first run
- `crates/agent-console-dashboard/src/config/mod.rs` - Export default config functionality
- `crates/agent-console-dashboard/src/cli/commands.rs` - Add `config init` or `config reset` command

### Dependencies

- [S007.01 - TOML Configuration Schema](./S007.01-toml-configuration-schema.md) -
  Schema defines what goes in the config
- [S007.02 - Configuration Loading](./S007.02-configuration-loading.md) - Loader
  determines when to create default
- [S007.03 - XDG Path Support](./S007.03-xdg-path-support.md) - Determines where to
  create the file
...
# - default: Two-line session-status + one-line api-usage (vertical stack)
# - compact: One-line session-status + one-line api-usage
