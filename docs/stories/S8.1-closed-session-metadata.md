...
- [S2.1 - Session Data Model](./S2.1-session-data-model.md) - Defines base
  session structure
- [S2.2 - Session Status Transitions](./S2.2-session-status-transitions.md) -
  Defines state machine
- [S2.4 - Session Lifecycle Events](./S2.4-session-lifecycle-events.md) -
  Handles session close events
- [S6.1 - Stop Hook Script](./S6.1-stop-hook-script.md) - Detects when session
  closes
...
- Detecting context limit exceeded (handled in S8.3)
- Actually resuming the session (handled in S8.2, S8.3)
