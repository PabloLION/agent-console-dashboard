...
Claude Code stores OAuth credentials in the macOS Keychain under service name `"Claude Code-credentials"`. The credential is a JSON object containing the access token needed for API calls. This story implements secure retrieval of these credentials on macOS using the `/usr/bin/security` CLI tool.

> **Note:** We use `/usr/bin/security` CLI instead of the `security-framework`
> crate because Claude Code hasn't added our app as a trusted keychain accessor.
> The crate approach would prompt users on every access. Using `/usr/bin/security`
> CLI reads the keychain without prompting because it inherits the user's
> security context.
...
1. Use `/usr/bin/security` CLI for Keychain access (avoids ACL prompts)
...
- `crates/claude-usage/Cargo.toml` - No additional dependencies needed
  (`/usr/bin/security` is a system binary)
...
pub fn get_token_macos() -> Result<String, CredentialError> {
    // Read from Keychain using /usr/bin/security CLI
    // This avoids ACL prompts that the security-framework crate would trigger
    ...
    let password_str = String::from_utf8(output.stdout)
        .map_err(|e| CredentialError::Parse(e.to_string()))?;
    let password_bytes = password_str.trim().as_bytes();
    ...
    if let Some(expires_at) = oauth.get("expiresAt").and_then(|v| v.as_i64()) {
        let now_secs = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs() as i64;  // Use seconds precision to avoid u128→i64 overflow risk

        // expiresAt is in milliseconds, convert to seconds for comparison
        if now_secs > expires_at / 1000 {
...
Using `/usr/bin/security` CLI avoids ACL prompts because the CLI tool inherits
the user's security context. The `security-framework` crate approach was
rejected because Claude Code hasn't added our app as a trusted keychain
accessor, which would cause prompts on every access.

### Security Practices

- Token retrieved → used immediately in API call → dropped
- No logging of token value (not even redacted)
- Token exists in memory only during the function call chain. "No storage beyond
  function scope" means no persistent caching — the token is fetched fresh each
  time and dropped after use
...
No additional crate dependencies needed for macOS. The `/usr/bin/security` CLI
is a system binary available on all macOS versions.
