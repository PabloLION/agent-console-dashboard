...
| Uninstall | `launchctl unload`, remove plist, confirm                            | `systemctl --user disable`, `systemctl --user stop`, remove unit, confirm |
...
1. Stop the running daemon (via platform-specific stop command)
2. Remove the service file (plist or unit file)
3. Confirm removal (check file no longer exists)
...
fn install_macos() -> Result<()> {
    ...
    // Copy plist from resources
    std::fs::copy("resources/com.agent-console.daemon.plist", &dest)?;
...
fn install_linux() -> Result<()> {
    ...
    // Copy unit file from resources
    std::fs::copy("resources/acd.service", &dest)?;
