...
**orchestrator** (maestro) — it conducts the symphony, never plays an instrument.
...
**permanent domain experts**, not single-use task workers. They can be
consulted on any issue in their field across multiple sessions.

Persistence:

- **Definition** (`.claude/agents/<name>.md`): git-tracked, permanent
- **Memory** (`.claude/agent-memory/<name>/MEMORY.md`): persists across sessions,
  first 200 lines auto-loaded on each invocation
- **Conversation**: starts fresh each invocation, but MEMORY.md provides continuity
...
1. Show the full issue (`bd show <id>`) including description, dependencies, etc.
...
deleted. Orchestrator reviews research output and discusses with user.
